...
let onNextDebugCommand: (() => void) | null = null
let disconnectCurrentSocket: (() => void) | null = null
...
function makeFakeSocket(responder: Responder): Socket {
  ...
  disconnectCurrentSocket = () => (handlers.disconnect || []).forEach((handler) => handler({ reason: 'transport close' }))
...
describe('WebSocketDebugTransport license function codes', () => {
  ...
  it('settles an in-flight request when the socket disconnects', async () => {
    currentResponder = () => []

    const transport = await connected()
    const commandSent = new Promise<void>((resolve) => {
      onNextDebugCommand = resolve
    })
    const resultPromise = transport.getVariablesList([1])
    await commandSent
    disconnectCurrentSocket?.()

    await expect(resultPromise).resolves.toEqual({ success: false, error: 'Socket disconnected' })
  })
})
