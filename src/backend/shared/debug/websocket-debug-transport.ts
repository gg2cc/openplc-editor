  ...
  private sendCommandNow<T>(
    ...
    return new Promise<T>((resolve, reject) => {
      ...
      const fail = (error: string) => {
        if (errorMode === 'reject' || !onFailure) reject(new Error(error))
        else resolve(onFailure(error))
      }
      ...
      const responseHandler = (response: { success: boolean; data?: string; error?: string }) => {
        ...
        if (!response.success) {
          finish(response.error || 'Unknown error')
          return
        }
        if (!response.data) {
          finish('No data in response')
          return
        }
      ...
      const timeoutHandle = setTimeout(() => finish('Request timeout'), REQUEST_TIMEOUT_MS)
